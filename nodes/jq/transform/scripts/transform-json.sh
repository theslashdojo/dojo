...
if is_true "${JQ_INPLACE:-false}" && [[ -z "${JQ_FILE:-}" ]]; then
  echo "Error: JQ_INPLACE requires JQ_FILE" >&2
  exit 1
fi
...
if is_true "${JQ_INPLACE:-false}"; then
  temp_file=$(mktemp "${JQ_FILE}.tmp.XXXXXX")
  cleanup() { rm -f "$temp_file"; }
  trap cleanup EXIT

  run_jq > "$temp_file"

  if [[ -n "${JQ_BACKUP_SUFFIX:-}" ]]; then
    cp "${JQ_FILE}" "${JQ_FILE}${JQ_BACKUP_SUFFIX}"
  fi

  mv "$temp_file" "${JQ_FILE}"
  trap - EXIT
else
  run_jq
fi
