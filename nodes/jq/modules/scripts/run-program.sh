...
if [[ -z "${JQ_PROGRAM_FILE:-}" ]]; then
  echo "Error: JQ_PROGRAM_FILE is required" >&2
  exit 1
fi

if [[ -n "${JQ_INPUT_FILE:-}" && -n "${JQ_INPUT:-}" ]]; then
  echo "Error: set only one of JQ_INPUT_FILE or JQ_INPUT" >&2
  exit 1
fi

if is_true "${JQ_NULL_INPUT:-false}" && [[ -n "${JQ_INPUT_FILE:-}" || -n "${JQ_INPUT:-}" ]]; then
  echo "Error: JQ_NULL_INPUT cannot be combined with JQ_INPUT_FILE or JQ_INPUT" >&2
  exit 1
fi
...
add_library_paths "${JQ_LIBRARY_PATH:-}"
add_named_json_vars "${JQ_VARS_JSON:-}"
JQ_ARGS+=(-f "${JQ_PROGRAM_FILE}")

if is_true "${JQ_NULL_INPUT:-false}"; then
  jq "${JQ_ARGS[@]}"
elif [[ -n "${JQ_INPUT_FILE:-}" ]]; then
  jq "${JQ_ARGS[@]}" "${JQ_INPUT_FILE}"
